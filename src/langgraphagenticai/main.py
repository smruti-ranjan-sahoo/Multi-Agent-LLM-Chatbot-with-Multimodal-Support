...
#             # Styled bubbles (stateless quick render)
...
#     # =====================================================
#     # STATEFUL MODES: Per-usecase conversation store
#     # =====================================================
#     if "conversations_by_usecase" not in st.session_state:
#         st.session_state.conversations_by_usecase = {}

#     if usecase not in st.session_state.conversations_by_usecase:
#         tid = str(uuid.uuid4())
#         st.session_state.conversations_by_usecase[usecase] = {
#             "current_thread_id": tid,
#             "threads": {
#                 tid: {"title": f"{usecase} chat", "messages": []}
#             }
#         }

#     uc_store = st.session_state.conversations_by_usecase[usecase]
#     threads = uc_store["threads"]
#     current_tid = uc_store["current_thread_id"]

#     # -----------------------------------------------------
#     # SIDEBAR – THREAD MANAGEMENT (per use case)
#     # -----------------------------------------------------
#     with st.sidebar:
#         st.divider()
...
#                 uc_store["current_thread_id"] = list(threads.keys())[0]
#                 current_tid = uc_store["current_thread_id"]
#                 st.session_state.image_bytes = None

#         # Thread picker (this usecase only)
...
#             f"{usecase} Threads",
#             list(titles.values()),
#             index=list(titles.keys()).index(current_tid)
#         )
...
#     # -----------------------------------------------------
#     # CURRENT THREAD (for THIS use case)
#     # -----------------------------------------------------
...
#             st.success("Document loaded. Ask a question about it!")

#     # -----------------------------------------------------
#     # MULTI-AGENT IMAGE UPLOAD (SCOPED TO THIS USE CASE)
#     # -----------------------------------------------------
...
#             st.image(st.session_state.image_bytes, use_container_width=True, caption="Image provided to the agent")

#         # Guard: require a vision model when an image is present
#         def _looks_like_vision_model(name: str) -> bool:
#             return any(k in (name or "").lower() for k in ("llama-4-scout", "llama-4-maverick"))

#         if st.session_state.image_bytes and selected_llm == "Groq" and not _looks_like_vision_model(selected_groq_model):
#             st.error(
#                 "Vision request detected, but the selected Groq model is text-only. "
#                 "Use `meta-llama/llama-4-scout-17b-16e-instruct` or "
#                 "`meta-llama/llama-4-maverick-17b-128e-instruct`."
#             )
#             return

#     # -----------------------------------------------------
#     # CHAT INPUT (STATEFUL USECASES)
#     # -----------------------------------------------------
#     col_chat, col_reset = st.columns([5, 1])
#     with col_chat:
#         user_message = st.chat_input("Type your message...")
#     with col_reset:
#         if st.button("🗑"):
...
#             usecase=usecase,
#             graph=graph,
#             messages=messages,                 # usecase-scoped thread messages
#             image=st.session_state.image_bytes
#         ).display_result_on_ui()


#updated main.py to include voice input and TTS response, as well as DB integration for conversation history. Also added better session management for retriever and image data.
# import streamlit as st
# import uuid
# import tempfile
...
#     # =====================================================
#     # STATEFUL MODES (Session + DB Hybrid)
#     # =====================================================
#     if "conversations_by_usecase" not in st.session_state:
#         st.session_state.conversations_by_usecase = {}
...
#         with c2:
#             if len(threads) > 1 and st.button("🗑 Delete"):
#                 db.delete_conversation(current_tid)
...
#     # -----------------------------------------------------
#     if not messages:
#         stored_messages = db.get_messages(current_tid)
...
#     #         st.success("Docume
#     # 
#     # nt loaded. Ask a question about it!")
# # -----------------------------------------------------
# # DOCUMENT QA (RAG)
# # -----------------------------------------------------
# # -----------------------------------------------------
...
#         if uploaded_file:
#             st.session_state.retriever = None##newly added line to reset retriever on new upload
#             suffix = "." + uploaded_file.name.split(".")[-1]
#             with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
#                 tmp.write(uploaded_file.read())
#                 file_path = tmp.name
...
#             # 🔥 Use thread_id as namespace (BEST PRACTICE)
#             namespace = current_tid

#             vectorstore = create_vector_store(chunks, namespace)
...
#         if st.session_state.image_bytes:
#             st.image(st.session_state.image_bytes, use_container_width=True, caption="Image provided")
...
#     # with col_chat:
#     #     user_message = st.chat_input("Type your message...")
#     # 🎙 Voice Mode Toggle (ADD HERE)
#     # 🎙 Voice Mode Toggle
#     # voice_mode = st.toggle("🎙 Voice Mode", value=False)

#     # col_chat, col_voice, col_reset = st.columns([4, 1, 1])

#     # user_message = None

#     # # 📝 Text input (only if voice OFF)
#     # if not voice_mode:
#     #     with col_chat:
#     #         user_message = st.chat_input("Type your message...")
#     voice_mode = st.toggle("🎙 Voice Mode", value=False)
...
#     col_voice, col_reset = st.columns([1, 1])


#     # 🎤 Voice input (only if voice ON)
#     if voice_mode:
...
#                 user_message = speech_to_text(temp_audio.name)
...
#         # Save user message
#         messages.append(HumanMessage(content=user_message))
#         db.add_message(current_tid, "user", user_message)
...
#             db.add_message(current_tid, "assistant", messages[-1].content)
...
#         # 🔊 Play voice
#         if messages and isinstance(messages[-1], AIMessage):
#             audio_path = text_to_speech(messages[-1].content)
#             st.audio(audio_path)
...
# from src.langgraphagenticai.graph.graph_builder import GraphBuilder
# from src.langgraphagenticai.ui.streamlitui.display_result import DisplayResultStreamlit
# from src.langgraphagenticai.auth import login_google, login_github, get_user
# # -------------------------
...
#     # Clear stale code from URL without processing it
#     st.query_params.clear()
...
#         url = login_google()
#         st.markdown(f"[👉 Click here to login with Google]({url})")

#     if st.button("Login with GitHub"):
#         url = login_github()
#         st.markdown(f"[👉 Click here to login with GitHub]({url})")
...
#     # ── Upload panel CSS (injected once) ────────────────────────────────────────
# UPLOAD_CSS = """
# <style>
# /* ── Upload zone card ── */
# .upload-card {
...
# }
# .voice-label {
#     font-size: 12px; color: #5A5F7A;
#     display: flex; align-items: center; gap: 6px;
# }
# </style>
# """

# # ── Helper: styled section divider ──────────────────────────────────────────
# def _section_header(icon: str, title: str, sub: str, icon_class: str = "doc-icon"):
#     return f"""
...
#         st.caption(f"👤 User: {user_id[:8]}")

#         # 🔐 LOGOUT
#         if st.button("🚪 Logout"):
#             # 1. Clear Supabase server session
#             try:
#                 supabase.auth.sign_out()
#             except:
#                 pass

#             # 2. Mark as logged out BEFORE clearing
#             logged_out_flag = True

#             # 3. Clear session
#             st.session_state.clear()

#             # 4. Set flags AFTER clear
#             st.session_state.user = None
#             st.session_state.auth_processed = False
#             st.session_state.logged_out = True  # ← KEY FLAG

#             # 5. Clear browser localStorage sb- tokens
#             components.html("""
#                 <script>
...
#     # ═══════════════════════════════════════════════════════════════════════
#     # DOCUMENT QA — styled upload panel
#     # ═══════════════════════════════════════════════════════════════════════
#     retriever = None
#     if "retriever" not in st.session_state:
#         st.session_state.retriever = None
...
#             # Show success pill inside the card
#             st.markdown(
...
#     # ═══════════════════════════════════════════════════════════════════════
#     # MULTI-AGENT — styled image upload panel
#     # ═══════════════════════════════════════════════════════════════════════
#     if "image_bytes" not in st.session_state:
#         st.session_state.image_bytes = None
...
#     # Text input — must be top-level (not inside columns)
#     if not voice_mode:
#         user_message = st.chat_input("Type your message…")

#     # Voice + Reset row
#     col_voice, col_spacer, col_reset = st.columns([2, 5, 1])

#     if voice_mode:
#         with col_voice:
#             if st.button("🎤 Record  (5 s)", use_container_width=True):
...
#             # generate ONLY if new response
#             if st.session_state.last_audio_id != current_audio_id:

#                 audio_path = text_to_speech(latest_text)

#                 st.session_state.last_audio_id = current_audio_id
#                 st.session_state.last_audio_path = audio_path

#             # play ONLY latest audio
#             if st.session_state.last_audio_path:
#                 st.audio(st.session_state.last_audio_path)

#Updated Audio_UI with caching to avoid regenerating TTS for same response. Unique ID based on hash of response text.
from distro import name
import streamlit as st
import uuid
import tempfile
