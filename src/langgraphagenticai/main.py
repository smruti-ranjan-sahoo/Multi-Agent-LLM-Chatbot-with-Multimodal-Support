...
def _voice_card_header():
    ...
    <div class="voice-card">
        <div class="voice-card-row">
            ...
            <div>
                <div class="voice-card-title">Voice Input</div>
                <div class="voice-card-sub">Record directly from your microphone</div>
            </div>
        </div>
    </div>"""


# ── TTS ✅ FIXED: cross-platform temp path ────────────────────────────────
def text_to_speech(text: str) -> str:
    tmp_dir   = tempfile.gettempdir()   # C:\...\AppData\Local\Temp on Windows, /tmp on Linux
    file_path = os.path.join(tmp_dir, f"response_{uuid.uuid4()}.mp3")
    gTTS(text).save(file_path)
    return file_path
...
def load_langgraph_agenticai_app():
    ...
    # ── VOICE mode ────────────────────────────────────────────────────
        ...
        # ✅ st.audio_input: records directly in browser — works on all deployments
        audio_input = st.audio_input(
            "🎙 Click the mic to record your message",
            key=f"voice_input_{current_tid}",
        )

        # ✅ Only transcribe when a NEW recording arrives
        if audio_input is not None:
            audio_bytes_data = audio_input.read()
            audio_id         = hash(audio_bytes_data)

            if audio_id != st.session_state.prev_audio_key:
                st.session_state.prev_audio_key = audio_id
                st.session_state.voice_sent     = False

                with st.spinner("🔄 Transcribing…"):
                    # ✅ Cross-platform temp file path
                    tmp_dir  = tempfile.gettempdir()
                    tmp_path = os.path.join(tmp_dir, f"voice_input_{uuid.uuid4()}.wav")
    ...
    if user_message:
        ...
        # ✅ TTS — fully cross-platform temp path
        if messages and isinstance(messages[-1], AIMessage):
            latest_text      = messages[-1].content
            current_audio_id = hash(latest_text)
            ...
            if st.session_state.get("last_audio_path"):
                # ✅ Verify file exists before playing (guards against session stale paths)
